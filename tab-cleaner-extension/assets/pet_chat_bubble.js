...
(() => {
  ...
  console.log('[Pet Chat Bubble] 🚀 pet_chat_bubble.js script loaded!');
  ...
  function getDialogueById(dialogues, id) {
    if (!dialogues || dialogues.length === 0) {
      console.warn('[Pet Chat Bubble] getDialogueById: dialogues array is empty');
      return null;
    }
    
    const dialogue = dialogues.find(d => d.id === id);
    if (!dialogue) {
      console.warn('[Pet Chat Bubble] getDialogueById: dialogue with id', id, 'not found');
      return null;
    }
    return dialogue;
  }
  ...
  function createChatBubbleStateMachine({
    ...
    async function showBubble(category = null, dialogueId = null, excludeIds = null) {
      console.log('[Pet Chat Bubble] showBubble called:', { category, dialogueId, excludeIds, isVisible, hasElements: !!(chatBubbleEl && textContentEl) });
      // 检查元素是否存在
      ...
      if (isVisible && excludeIds !== null && excludeIds.length === 0) {
        // 手动触发时，允许覆盖当前显示
        console.log('[Pet Chat Bubble] Overriding visible bubble for manual trigger');
        isVisible = false;
      } else if (isVisible) {
        // 自动显示时，如果已显示则跳过
        console.log('[Pet Chat Bubble] Already visible, skipping');
        return;
      }
      
      // 确保对话数据已加载
      if (dialogues.length === 0) {
        console.log('[Pet Chat Bubble] Loading dialogue data...');
        dialogues = await loadDialogueData(assetFn);
        console.log('[Pet Chat Bubble] Loaded', dialogues.length, 'dialogues');
      }
      ...
      if (dialogueId !== null) {
        ...
        console.log('[Pet Chat Bubble] Found dialogue by ID:', dialogue);
      } else {
        ...
        console.log('[Pet Chat Bubble] Selected random dialogue:', dialogue);
      }
      
      if (!dialogue) {
        console.warn('[Pet Chat Bubble] No dialogue found!');
        return;
      }
      
      textContentEl.textContent = dialogue.content || '...';
      console.log('[Pet Chat Bubble] Set text content:', dialogue.content);
      
      // 显示动画（同步缩小 scale 到 65%）
      chatBubbleEl.style.display = 'block';
      chatBubbleEl.style.opacity = '0';
      chatBubbleEl.style.transform = 'translateY(6px) scale(0.585)'; /* scale 从 0.9 缩小到 0.585 (0.9 * 0.65) */
      ...
      requestAnimationFrame(() => {
        ...
        chatBubbleEl.style.transform = 'translateY(0) scale(0.65)'; /* scale 从 1 缩小到 0.65 (1 * 0.65) */
      });
    ...
    function triggerShow(category = null, dialogueId = null) {
      console.log('[Pet Chat Bubble] triggerShow called:', { category, dialogueId, isVisible, hasElements: !!(chatBubbleEl && textContentEl) });
      clearTimers();
      ...
      // 直接调用 showBubble，不需要 setTimeout（showBubble 是 async 但不需要等待）
      console.log('[Pet Chat Bubble] Calling showBubble:', { category, dialogueId, isVisible });
      showBubble(category, dialogueId, []).catch(err => {
        console.error('[Pet Chat Bubble] Error in showBubble:', err);
      });
    }
