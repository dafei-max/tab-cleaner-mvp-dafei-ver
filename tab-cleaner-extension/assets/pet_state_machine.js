(() => {
  console.log('[Pet State Machine] 🚀 pet_state_machine.js script loaded!');
  ...
  function createPetStateMachine({
    ...
    function startRoaming() {
      ...
      // 从 transform 或 left/top 中获取当前位置
      let currentLeft, currentTop;
      const transform = petContainer.style.transform;
      if (transform && transform.includes('translate3d')) {
        const match = transform.match(/translate3d\(([^,]+)px,\s*([^,]+)px/);
        if (match) {
          currentLeft = parseFloat(match[1]);
          currentTop = parseFloat(match[2]);
        }
      }
      if (currentLeft === undefined) {
        currentLeft = parseFloat(petContainer.style.left || `${(viewportW - petW) / 2}`);
      }
      if (currentTop === undefined) {
        currentTop = parseFloat(petContainer.style.top || `${(viewportH - petH) / 2}`);
      }
      ...
      requestAnimationFrame(() => {
        // ⚡ 使用 transform 替代 left/top（性能优化）
        petContainer.style.transition = `transform ${moveDuration}ms ease-in-out`;
        petContainer.style.transform = `translate3d(${targetLeft}px, ${targetTop}px, 0)`;
        // 同时更新 left/top 用于位置保存
    ...
    function setState(state, options = {}) {
      console.log('[Pet State Machine] setState called:', { state, options, petId: getPetId?.() });
      
      if (getPetId?.() !== 'elephant') {
        console.warn('[Pet State Machine] Not elephant, skipping state change');
        return;
      }
      const avatarVideoEl = videoElRef?.();
      if (!avatarVideoEl) {
        console.warn('[Pet State Machine] Video element not found');
        return;
      }
      ...
      console.log('[Pet State Machine] Video source:', src);
      ...
      console.log('[Pet State Machine] Need switch video?', needSwitch, 'current:', avatarVideoEl.dataset.currentSrc, 'new:', src);
      
      if (needSwitch) {
        avatarVideoEl.pause();
        avatarVideoEl.src = src;
        avatarVideoEl.dataset.currentSrc = src;
        console.log('[Pet State Machine] Video source switched to:', src);
      }
   ...
   * - 触发 id:26 的气泡对话
   *
   * 事件监听仍由外部模块绑定，这里只负责状态机和对话逻辑。
   */
  function handleAvatarClick(petFsm, petStates) {
    console.log('[Pet State Machine] handleAvatarClick called', {
      hasPetFsm: !!petFsm,
    ...
    if (petFsm && petStates && petStates.QUESTION) {
      try {
        ...
        console.warn('[Pet State Machine] Failed to set QUESTION state on click:', err);
      }
    }
    ...
    if (window.__TAB_CLEANER_PET_CHAT_BUBBLE && window.__TAB_CLEANER_PET_CHAT_BUBBLE.trigger) {
      try {
        ...
        console.warn('[Pet State Machine] Failed to trigger chat bubble on click:', err);
      }
    }
  }
