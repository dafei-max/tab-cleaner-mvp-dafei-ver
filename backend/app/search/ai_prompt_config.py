...
1. **Caption（图片描述）**：
   - **严格限制：不超过 100 字**，但必须足够精确和完整
   - **必须包含以下所有元素**（按优先级）：
     a) **视觉主体**：图片中的主要物体或人物是什么（如：椅子、人物、食物、建筑等）
     b) **主要颜色**：主体物体的颜色（如：黄色、红色、蓝色等），必须准确描述
     c) **背景**：背景颜色和主要背景元素（如：白色背景、蓝色天空、木质桌面等）
     d) **人物行为**（如果有人的话）：人物在做什么（如：坐着、站着、走路、微笑等）
     e) **物体特征状态**（如果有物的话）：物体的形状、材质、状态、特征（如：圆形、木质、融化、拉丝、现代风格等）
...
{
  "caption": "图片描述（不超过100字，必须包含：视觉主体+主要颜色+背景+人物行为或物体特征状态）",
...
**示例**：
- 如果图片是一个黄色烤奶酪三明治，caption 应该是："一个黄色的烤奶酪三明治，融化的奶酪拉出长长的丝，放在蓝色背景的盘子上，整体呈现温暖的食物风格。"
  - 包含：视觉主体（三明治）+ 主要颜色（黄色）+ 背景（蓝色）+ 物体特征状态（融化、拉丝）
  - dominant_colors 应该包含：["yellow", "blue"]
- 如果图片是一个红色现代椅子，caption 应该是："一把红色的现代风格椅子，采用简约设计，木质材质，放置在白色背景的室内空间中。"
  - 包含：视觉主体（椅子）+ 主要颜色（红色）+ 背景（白色）+ 物体特征状态（现代、简约、木质）
  - dominant_colors 应该包含：["red", "white"]
- 如果图片是一个人坐在椅子上，caption 应该是："一个穿着蓝色衣服的人坐在棕色的木椅上，面带微笑，背景是白色的墙壁和绿色的植物。"
  - 包含：视觉主体（人）+ 主要颜色（蓝色、棕色）+ 背景（白色、绿色）+ 人物行为（坐着、微笑）
  - dominant_colors 应该包含：["blue", "brown", "white", "green"]"""
    else:
