"""
批量生成 Caption 脚本（修复版，含 WebSocket 通知）
...
from search.caption import batch_enrich_items  # noqa: E402
from search.embed import embed_text  # noqa: E402
from search.qwen_vl_client import QwenVLClient  # noqa: E402
from search.config import get_api_key  # noqa: E402


def to_vector_str(vec: Optional[List[float]]) -> Optional[str]:
    """将向量列表转换为数据库格式（Postgres vector 文本）"""
    if not vec:
        return None
...
async def get_items_without_caption(
    ...
    if has_new_fields:
        if user_id:
            ...
            if force_all:
                where_clause = """WHERE user_id = $1 AND status = 'active'
...
async def update_item_caption(
    ...
    """更新数据库中的 Caption 和视觉属性，并可选推送 WebSocket"""
    pool = await get_pool()
    user_id = _normalize_user_id(user_id)
        ...
        async with pool.acquire() as conn:
            ...
            if send_websocket:
                try:
                    ...
                    print("[BatchEnrich] ⚠️ WebSocket not available (not in FastAPI context)")
...
示例:
  # 处理所有用户的数据（最多 100 项）
  python batch_enrich_captions_fixed.py

  # 只处理特定用户的数据
  python batch_enrich_captions_fixed.py --user-id user123

  # 不发送 WebSocket 通知（静默模式）
  python batch_enrich_captions_fixed.py --no-websocket
        """,
    )
    ...
    parser.add_argument("--no-websocket", action="store_true", help="不发送 WebSocket 通知")
    ...
    print(f"  - 发送 WebSocket 通知: {not args.no_websocket}")
    print("=" * 60)
    ...
    try:
        ...
        stats = await process_batch(
            ...
            send_websocket=not args.no_websocket,
        )
